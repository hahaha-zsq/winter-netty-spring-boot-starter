...
public class WebSocketHandshakeAuthHandler extends SimpleChannelInboundHandler<FullHttpRequest> {
     ...
     * @param fullHttpRequest 接收到的消息对象
    ...
    protected void channelRead0(ChannelHandlerContext channelHandlerContext, FullHttpRequest fullHttpRequest) throws Exception {
        if (isWebSocketHandshake(fullHttpRequest)) {
            // 执行认证逻辑
            if (!handleWebSocketAuth(channelHandlerContext, fullHttpRequest)) {
                // 认证失败，不继续传递请求到后续处理器
